...
def check_dates(dates):
    ...
    if len(dates)==2: 
        ...
        dates = [dates[0][0], dates[-1][1]]

    else:

        raise ValueError(f"dates have a not comforming format, type: {type(dates)} , length: {len(dates)}, values: {dates}")
